...
type Subject = 'BWL' | 'Datenbanken' | 'Java' | 'SAP';
...
const INITIAL_TOPICS: Record<Subject, Topic[]> = {
  BWL: [
    { id: 'bwl-1', title: 'Marketing-Mix (4 Ps: Product, Price, Place, Promotion)', createdAt: new Date(Date.now() - 3600000 * 24 * 3).toISOString() },
    { id: 'bwl-2', title: 'Rechtsformen (GmbH, AG, Einzelunternehmen, OHG)', createdAt: new Date(Date.now() - 3600000 * 24 * 1.5).toISOString() },
    { id: 'bwl-3', title: 'Deckungsbeitragsrechnung & Break-Even-Point Analyse', createdAt: new Date(Date.now() - 3600000 * 6).toISOString() }
  ],
  Datenbanken: [
    { id: 'db-1', title: 'ER-Modellierung & Kardinalitäten (1:1, 1:n, m:n)', createdAt: new Date(Date.now() - 3600000 * 24 * 4).toISOString() },
    { id: 'db-2', title: 'SQL-Abfragen (JOINs, GROUP BY, HAVING, Unterabfragen)', createdAt: new Date(Date.now() - 3600000 * 24 * 1).toISOString() },
    { id: 'db-3', title: 'Normalisierung von Relationen (1. bis 3. Normalform)', createdAt: new Date(Date.now() - 3600000 * 4).toISOString() }
  ],
  Java: [
    { id: 'java-1', title: 'Prinzipien der OOP (Vererbung, Polymorphie, Kapselung)', createdAt: new Date(Date.now() - 3600000 * 24 * 5).toISOString() },
    { id: 'java-2', title: 'Fehlerbehandlung mittels Try-Catch & Custom Exceptions', createdAt: new Date(Date.now() - 3600000 * 18).toISOString() }
  ],
  SAP: [
    { id: 'sap-1', title: 'Organisationsstrukturen im SAP ERP (Mandant, Buchungskreis, Werk)', createdAt: new Date(Date.now() - 3600000 * 24 * 2).toISOString() },
    { id: 'sap-2', title: 'SD-Prozesskette: Von der Kundenanfrage bis zum Zahlungseingang', createdAt: new Date(Date.now() - 3600000 * 2).toISOString() }
  ]
};

export default function Home() {
  const [topics, setTopics] = useState<Record<Subject, Topic[]>>({
    BWL: [],
    Datenbanken: [],
    Java: [],
    SAP: []
  ...
  useEffect(() => {
    ...
    // Load Topics and validate format to prevent crashes from old/corrupted data
    const savedTopics = localStorage.getItem('exam_topics');
    if (savedTopics) {
      try {
        ...
        const isValid = parsed && 
                        ...
                        SUBJECTS.every(sub => Array.isArray(parsed[sub]));
        
        if (isValid) {
          setTopics(parsed);
        } else {
          console.warn("Invalid topics format in localStorage, resetting to defaults.");
          setTopics(INITIAL_TOPICS);
          localStorage.setItem('exam_topics', JSON.stringify(INITIAL_TOPICS));
        }
      } catch (e) {
        console.error("Error parsing saved topics, fallback to default", e);
        setTopics(INITIAL_TOPICS);
        localStorage.setItem('exam_topics', JSON.stringify(INITIAL_TOPICS));
      }
    } else {
  ...
  const handleAddTopic = (e: React.FormEvent) => {
    ...
    const newTopic: Topic = {
      id: `${activeSubject.toLowerCase()}-${Date.now()}`,
      title: text,
    ...
    const updatedTopics = {
      ...topics,
      [activeSubject]: [newTopic, ...(topics[activeSubject] || [])]
    };

    setTopics(updatedTopics);
    localStorage.setItem('exam_topics', JSON.stringify(updatedTopics));
    setNewTopicText('');
  ...
  const handleDeleteTopic = (subject: Subject, id: string) => {
    const updatedTopics = {
      ...topics,
  ...
  // Render static mockup before mount to avoid Next.js hydration issues
  if (!isMounted) {
    return (
    ...
    <div className="relative min-h-screen w-full flex flex-col justify-start">
      ...
      <footer className="mt-16 pt-6 border-t border-card-border text-center text-xs text-text-secondary">
        ...
        <p className="mt-1 opacity-70">
          Clientseitig gespeichert in localStorage (keine Cookie-Zustimmung erforderlich) &bull; Tastatur-Navigierbar
        </p>
      </footer>
      </div>
    </div>
